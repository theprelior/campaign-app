...
export default function InfluencersPage() {
  ...
  return (
    <div className="container mx-auto p-8">
      <h1 className="text-3xl font-bold">Influencer Yönetimi</h1>
        ...
        <div className="lg:col-span-1">
          <form onSubmit={handleSubmit} className="rounded-lg border p-6 shadow-sm">
            <h2 className="text-xl font-semibold mb-4">
                {editingInfluencer ? "Influencer Düzenle" : "Yeni Influencer Ekle"}
            </h2>
            <div className="flex flex-col gap-4">
              <input type="text" placeholder="İsim" value={name} onChange={e => setName(e.target.value)} required className="w-full rounded-lg border px-4 py-2" />
              <input type="number" placeholder="Takipçi Sayısı" value={followers} onChange={e => setFollowers(e.target.value)} required className="w-full rounded-lg border px-4 py-2" />
              <input type="number" step="0.01" placeholder="Etkileşim Oranı (%)" value={engagement} onChange={e => setEngagement(e.target.value)} required className="w-full rounded-lg border px-4 py-2" />
              <div className="flex gap-2">
                <button type="submit" disabled={createMutation.isPending || updateMutation.isPending} className="rounded-lg bg-blue-600 px-6 py-2 font-semibold text-white disabled:bg-gray-400">
                    {editingInfluencer ? "Güncelle" : "Ekle"}
                </button>
                {editingInfluencer && (
                    <button type="button" onClick={handleCancelEdit} className="rounded-lg bg-gray-500 px-6 py-2 font-semibold text-white">İptal</button>
                )}
              </div>
            </div>
          </form>
          ...
          <div className="overflow-x-auto rounded-lg border">
            <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                <thead className="text-left">
                    <tr>
                        <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">İsim</th>
                        <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Takipçi</th>
                        <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Etkileşim (%)</th>
                        <th className="px-4 py-2"></th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                    {isLoading && <tr><td colSpan={4} className="text-center p-4">Yükleniyor...</td></tr>}
                    {influencers?.map(inf => (
                        <tr key={inf.id}>
                            ...
                            <td className="whitespace-nowrap px-4 py-2">
                                <div className="flex gap-2">
                                    <button onClick={() => handleEdit(inf)} className="text-yellow-600 hover:underline">Düzenle</button>
                                    <button onClick={() => handleDelete(inf.id)} className="text-red-600 hover:underline">Sil</button>
                                </div>
                            </td>
                        </tr>
                    ))}
